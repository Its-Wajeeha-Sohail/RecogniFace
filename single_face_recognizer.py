...
def get_face_embeddings(image):
    ...
    if faces is not None:
        # Resize face images to match FaceNet model input size (160x160)
        resized_faces = []
        for face in faces:
        ...
        resized_faces = np.transpose(resized_faces, (0, 3, 1, 2))  # Transpose to (N, C, H, W)
...
while True:
    ...
    if frame_embeddings is not None:
        ...
        for known_emb in known_embeddings:
            dist = np.linalg.norm(known_emb - frame_embeddings, axis=1)
        ...
        # Display the recognized label and distance on the frame
        cv2.putText(frame, f"{recognized_label} - {min_distance:.2f}", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
